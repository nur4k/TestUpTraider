...
def draw_menu(context: RequestContext, name: str = '', parent: int = 0):
    ...
    if parent != 0 and 'menu' in context:
        ...
        current_path = context['request'].path \
            if 'request' in context and isinstance(context['request'], HttpRequest) else ''

        data = TreeMenu.objects.select_related().filter(category__name=name).order_by('pk')
        ...
        for item in data:
            ...
            if is_url.match(path):
                ...
                try:
                    url = reverse(path)
                except Exception('URL incorrect!'):
                    url = path
